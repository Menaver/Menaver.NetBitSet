    ...
    public partial class NetBitSet
    {
        public static NetBitSet operator +(NetBitSet left, NetBitSet right)
        {
            ...
            if (left.WordLength == right.WordLength)
                ...
                if (right.WordLength % left.WordLength == 0)
                newWordLength = left.WordLength;

            NetBitSet result = new NetBitSet(checked(left.Count + right.Count), newWordLength, 0);
        ...
        public static NetBitSet operator <<(NetBitSet obj, int count)
        {
            ...
            NetBitSet result = (NetBitSet)obj.Clone();
        ...
        public static NetBitSet operator &(NetBitSet left, NetBitSet right)
        {
            ...
            NetBitSet result = (NetBitSet)left.Clone();
            for (var i = 0; i < left.Count; i++)
        ...
        public static bool operator <(NetBitSet left, NetBitSet right)
        {
            ...
            for (var i = 0; i < left._container.Count; i++)
            {
                ...
                if (left._container[i] && !right._container[i]) return false;   // if (1 and 0) then (left > right) -> return false
                if (!left._container[i] && right._container[i]) return true;    // if (0 and 1) then (left < right) -> return true
            }
        ...
        public static bool operator >(NetBitSet left, NetBitSet right)
        {
            ...
            for (var i = 0; i < left._container.Count; i++)
            {
                ...
                if (left._container[i] && !right._container[i]) return true;   // if (1 and 0) then (left > right) -> return true
                if (!left._container[i] && right._container[i]) return false;    // if (0 and 1) then (left < right) -> return false
            }
